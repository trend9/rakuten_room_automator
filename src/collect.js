...
function generateFallbackMessage(title) {
  ...
  if (lower.includes('スイーツ') || lower.includes('ケーキ') || lower.includes('チョコ') || lower.includes('和菓子') || lower.includes('洋菓子') || lower.includes('お菓子') || lower.includes('ゼリー') || lower.includes('ティラミス')) {
    intros = [
      'お取り寄せで話題になっている一品。製法・素材・味のバランスが突き抜けていて、贈り物にも自分用にも選ばれる理由がわかる。✨🍫',
      '楽天の食品ランキングで常連入りしているスイーツ。一口食べると素材のクオリティが伝わってくる、丁寧に作られた逸品だ。🏆🍰',
      '見た目のインパクトだけでなく味の完成度が高い。ギフトとして送っても恥ずかしくない、信頼できる品質のお取り寄せスイーツ。🎁✨',
    ];
    tagSets = [
    ...
    intros = [
      'サロン通いのコストを考えると、この性能のホームケア機器は長期的に見てコスパが圧倒的に優れている。✨📊',
      '技術仕様が本格的で、家庭用とは思えない出力設計になっている。毎日のルーティンに組み込む価値がある美容家電だ。🔬⚡',
      '口コミ評価の数と内容を見れば実力は一目瞭然。使い続けることで違いが出てくる、投資する価値のある一台。📈🎯',
    ];
    tagSets = [
    ...
    intros = [
      '家事の時間をここまで削減できる家電は貴重だ。導入前後の生活の変化を体感した人が口を揃えてリピートしている理由がある。⏱️🏠',
      '設計の合理性が高く、機能と使いやすさの両立ができている。忙しい生活に本当に役立つ、実用的な選択肢だ。🔧✅',
      '同価格帯での比較優位が明確。スペックと使い勝手のバランスが取れた、買って後悔しない家電の一つ。📊🎯',
    ];
    tagSets = [
    ...
    intros = [
      'スペックと耐久性のバランスが突出している。フィールドで実際に使ってみると、設計思想の丁寧さが随所に感じられる。⛺🔥',
      'アウトドアギアは信頼性が全て。このアイテムはその点で評価が高く、実際に使うシーンを想定した設計になっている。🏕️✅',
      '同ジャンルの他製品と比較して、コストパフォーマンスが頭一つ抜けている。本格的に使い込める道具だ。🎣📊',
    ];
    tagSets = [
    ...
    intros = [
      '楽天での評価数と内容を精査したうえで選んだ一品。機能・デザイン・耐久性のどれを見ても納得の水準に達している。🔍✅',
      '使い勝手の良さが口コミで広がっている商品。実際に手にすると素材感や作りの良さが伝わってくる。📦🎯',
      'コストパフォーマンスの高さが際立つアイテム。この価格帯でこの品質は、調べるほど納得感が増す。💡📊',
    ];
    tagSets = [
  ...
  const ctaList = [
    '\n\n詳細スペックや最新価格・ポイント情報は楽天市場の商品ページで確認できる。在庫状況も合わせてチェック推奨。📦',
    '\n\n実際のユーザーレビューや最安値情報は楽天市場で公開中。購入前に評価数と内容をしっかり確認しておこう。🔍',
    '\n\n期間限定のポイント還元やクーポン情報は楽天市場公式ページに掲載されている。損しないうちに確認を。🏷️',
  ];
      ...
      try {
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: "あなたは楽天ROOMで活動する男性インフルエンサーです。商品の機能・性能・コスパを的確に解説する口調で、読者が『これは本当に良さそうだ』と感じる日本語のみで執筆してください。ギャルっぽい語尾・♡などのハート系絵文字・過剰な感嘆符は使わないこと。\n\n" + prompt
              }]
            }],
            generationConfig: {
              temperature: 0.7,
    ...
    try {
      ...
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: model,
          messages: [
            { role: "system", content: "あなたは楽天ROOMで活動する男性インフルエンサーです。商品の機能・スペック・コストパフォーマンスを分析的かつ自然な日本語で解説してください。ギャルっぽい口調・ハート系絵文字（♡・💕・💗など）・「〜だよ！」「〜なんです♪」のような女性語は使わないこと。" },
    ...
    try {
      ...
      const res = await fetch(`https://api-inference.huggingface.co/models/${model}/v1/chat/completions`, {
        ...
        body: JSON.stringify({
          model: model,
          messages: [
            { role: 'system', content: 'あなたは楽天ROOMで活動する男性インフルエンサーです。商品の機能・スペック・コスパを分析的かつ自然な日本語で解説してください。ギャルっぽい口調・ハート系絵文字（♡・💕・💗など）・「〜だよ！」「〜なんです♪」のような女性語は使わないこと。' },
            { role: 'user', content: prompt }
          ],
          max_tokens: 500,
          temperature: 0.7
...
- 口調：「〜だ。」「〜できる。」「〜がある。」など落ち着いた解説調。断定口調でOK
- 絵文字を3〜5個使う（✅📦🔍📊⚡🏆🎯💡のような実用的な絵文字を優先）
- ⚠️ 絶対禁止：♡💕💗などのハート系絵文字
- ⚠️ 絶対禁止：「〜だよ！」「〜なんです♪」「〜ちゃいます」などギャル・女性語調の語尾
- ⚠️ 絶対禁止：「PR」「広告」の記載
...
- 最後は「詳細は楽天市場で確認できる」など購入導線につながる一文で締める
